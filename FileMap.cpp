...
namespace MuffleSpeech {
	void FindFiles(FileMap* aFileMap) {
		...
		try {
			...
			std::cout << "\n"
			          "./profiles/ folder was not found, creating ./profiles/ folder.\n";
			std::filesystem::create_directories("./profiles/");
		}
	}
	void PrintFiles(const FileMap* aFileMap) {
		std::putchar('\n');
		for(uint64_t i = 0; i < aFileMap->Map.size(); i++) {
	...
	uint64_t SelectFile(const FileMap* aFileMap) {
		uint64_t lSelection;
		while(true) {
			std::cout << "Please select profile from above options. Input 0 or any letter/string to exit.\nmfsp>";
			std::cin >> lSelection;
			if(lSelection == 0) {
